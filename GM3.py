...
if os.path.exists("GMCMI"):
    ...
    if os.path.exists("ign.txt"):
        ...
        print(Fore.YELLOW +f"By running this tool, your entire mods folder will be deleted to ensure compatibilty with the server. To supress this warning on startup, create a file named [ign.txt] in the {rootDir}\.minecraft\\GMCMI\\")
        print(Style.RESET_ALL)
...
try:
    if ver == curVer: # Compares what version we have installed
        ...
        print(Style.RESET_ALL)
        print(Fore.MAGENTA + f"Server: {ver}")
        print(Style.RESET_ALL)
        print(Fore.GREEN + f"Client: {curVer}")
        print(Style.RESET_ALL)
        os.remove("ver.txt")
    ...
    print(Fore.YELLOW + "Unable to locate a previous update version. Installing anyway.")
    print(Style.RESET_ALL)
